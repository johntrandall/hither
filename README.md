...
The LaunchAgent runs in user GUI context (required for Keychain access). It calls the DSM Web API **as the target user**, which means the server-side ACL filter returns exactly the shares that user can read. The script renders an autofs indirect-map body and pipes it through a root-owned wrapper script that atomically writes `/etc/hither_<nas>` (mode 0600) and runs `automount -cv`. The map line embeds a URL-encoded copy of the DSM password — `mount_smbfs` cannot reliably consume CLI-created Keychain entries, so the fully-automated path requires the credential to live in the map file itself; see [docs/design-decisions.md](docs/design-decisions.md#why-etchither_nas-contains-url-encoded-passwords-on-disk) for the threat model.
