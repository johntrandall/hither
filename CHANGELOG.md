...
### Fixed
- **Single-flight lock in `hither sync` (`libexec/hither-sync.sh`).** Two concurrent sync invocations (the 04:23 LaunchAgent firing while a user runs a manual `hither sync`, or a teammate firing both) raced through the read-DSM-then-write-map sequence with no guard. The wrapper's per-call write is atomic, but the surrounding sequence isn't — both callers would spend a DSM auth token and the notification path could trip twice for the same change. Adopted the `bootstrap/apply-auto-master.sh` mkdir-mutex pattern (atomic mkdir + 600s stale-TTL break + EXIT-trap release). Lock lives at `${HOME}/Library/Caches/hither/sync.lock` — user-writable; placing it under root-owned `/var/run/` would require sudo just to acquire, defeating the point. A second caller exits 0 (graceful no-op, not an error condition) and logs why. Test override knobs: `HITHER_LOCK_DIR` repoints the lock dir; `HITHER_SKIP_LOCK=1` disables it entirely.
- **Tempfile orphan in `sbin/hither-write-map` on SIGKILL.** The wrapper writes `${target}.tmp.$$` then `mv -f`. Each error branch already cleans up its own path, but a SIGKILL between the head-c-write and the rename would leave `/etc/hither_<host>.tmp.<pid>` orphaned forever. Added an EXIT trap immediately after target/tempfile declaration. After a successful rename(2), the tempfile inode no longer lives at the tempfile name — the trap's `rm -f` on the post-rename non-existent path is harmless.
- **osascript-wedged log lie in `fire_notification` (`libexec/hither-sync.sh`).** The perl-alarm wrapper around osascript was followed by an unconditional `log "notification fired"`, so a wedged Notification Center (alarm fires; delivery never happened) produced a success line in the sync log. Now captures the exit status: `notification dispatched: …` on success vs `[warn] notification dispatch timed out or failed for …` on failure. Operators debugging "I never got a notification" can now distinguish never-fired from delivery-failed.
- **Stale-Keychain → silent failure → notification (`libexec/hither-sync.sh`).** `dsm_login` failure logged a single line in `~/Library/Logs/hither/sync.log` and produced zero user-visible signal; users discovered the problem only when a stale `cd /Hither/<nas>/<share>` tripped a `No such file or directory`. When `HITHER_NOTIFY=1` is set, an auth failure now fires a macOS notification with the actionable recovery hint (`hither subscribe <nas> --user <dsm-user>` to re-prompt). Gated on opt-in so silent-mode users keep their silent mode.
- **README reboot vs `apfs.util -t` contradiction.** The "Install" section said "After a one-time reboot to materialize the `/Hither` synthetic root" — but `bootstrap/add-synthetic-root.sh` uses `apfs.util -t` precisely to avoid the reboot, which is one of the architecture's selling points. Rewrote the sentence to match: no reboot needed in normal cases; reboot is the fallback if `apfs.util -t` doesn't take. Also fixed the same contradiction in `Formula/hither.rb`'s `caveats` block.
- **README notification opt-in semantics block.** Added a "How it works" paragraph spelling out: (a) v0.5.0+ subs default `notify_on_changes = true`; (b) v0.4.x-upgraded subs default `false` because the field is absent and `hither_sub_read_notify` returns false for absent; (c) `HITHER_NOTIFY` is a single global env var on the LaunchAgent, OR-aggregated across all subscriptions — so adding ONE new sub with default-true notify silently enables notifications for ALL existing subs on the same Mac (including those upgraded from v0.4.x). Operators who want a sub to stay silent must set `notify_on_changes = false` in the TOML explicitly or use `--notify=false` on `hither subscribe`. The per-NAS gate is a post-v1.0 item.

### Added
- **`tests/test-sync-lock.sh`** — verifies (a) acquiring the lock prevents a second acquire (concurrent fire exits 0); (b) a lock dir mtime'd >600s in the past is broken and re-acquired; (c) the EXIT trap releases the lock after a successful run. Uses `HITHER_LOCK_DIR` to point at a per-test tempdir so the real user lock is never touched.

### Deferred to v0.6
- **`cmd_unsubscribe` privileged-write of `/etc/auto_master`.** `bin/hither cmd_unsubscribe` currently shells out to `sudo -n sed -i.bak '/^\\/Hither\\/<nas>[[:space:]]/d' /etc/auto_master` directly, bypassing the wrapper's "single privileged surface" principle (the whole point of `sbin/hither-write-map` is that it's the *only* sudoers grant). Fixing this requires extending the wrapper to handle remove operations OR adding a separate `hither-remove-host-line` wrapper, plus a sudoers update. ≥30 LOC of careful work; not v0.5.3 scope.
- **Mid-task `cmd_subscribe` rollback trap.** `cmd_subscribe` runs 7 mutating steps (Keychain write → TOML write → auto_master apply → LaunchAgent refresh → initial sync …). No `trap` chains them into an atomic operation, so a SIGKILL between Keychain-write and TOML-write leaves an orphan Keychain entry. v0.6 will add trap-based rollback (per-step inverse, fired in reverse order on signal).
- **WatchPaths semantics on Sonoma+.** The LaunchDaemon relies on `WatchPaths` to detect `/etc/auto_master` and `/etc/synthetic.conf` reverts. macOS may have changed kqueue `NOTE_WRITE` behavior for atomic-rename writes vs in-place writes on Sonoma+. Currently untested against the matrix. v0.6 will add a macOS-version test matrix that synthesizes both write patterns and asserts the daemon fires.

### Version
- `bin/hither` `HITHER_VERSION` → `0.5.3`
- `Formula/hither.rb` `version` → `0.5.3`
