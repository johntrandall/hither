...
#
# WORKFLOW:
#   1. Edit ~/dev/hither/server/hither-sync.{sh,manifest.json}
#   2. git commit + git push umbridge main  (Pattern #2: SHA must be on Forgejo)
#   3. ./register-events.sh                # dry-run, see plan
#   4. ./register-events.sh --apply        # create/update events
#   5. (Verify a manual run on each host — Run Now from the xyOps UI)
#   6. ./register-events.sh --apply --with-tickets
...
# Canonical path within the dev/hither repo on Forgejo:
...
echo "Pinning to dev/hither HEAD: $SHA"
...
build_wrapper() {
  ...
  jq -nr \
    ...
    "#\n" +
    "# Canonical script: dev/hither@" + $sha + ":" + "server/hither-sync.sh\n" +
    "# Updated by: server/register-events.sh (in the dev/hither repo)\n" +
    "# Token source: xyOps secret '\''forgejo-ro-admin-technical'\'' → env " + $secret + "\n" +
    "#   (Secret title is a legacy name — it now holds a Forgejo read-only token\n" +
    "#    that has access to both dev/hither and infra/admin-technical orgs.)\n" +
    "# Target DSM user: " + $user + "  |  NAS list: " + $nas_list + "\n" +
    "#\n" +
    "# To deploy a new version: edit ~/dev/hither/server/hither-sync.sh OR hither-sync.manifest.json,\n" +
    "# commit, `git push umbridge main`, then `./register-events.sh --apply`.\n" +
    "set -euo pipefail\n" +
    ...
    "# Clean the tempfile on any exit path. Note: `exec` REPLACES this shell\n" +
    "# with the target script, so the trap fires only on failure paths before\n" +
    "# exec (curl failure, chmod failure). Once exec runs, the target script\n" +
    "# owns its own lifecycle and the tempfile remains until cleaned by tmp\n" +
    "# reaper — acceptable; periodic /tmp purge handles it.\n" +
    "trap 'rm -f \"$TMP\"' EXIT\n" +
  ...
  if [ -z "$existing_id" ]; then
    ...
    body=$(jq -n \
      ...
      '{
         ...
         actions: $actions,
         limits: $limits,
         notes: ("Auto-generated by server/register-events.sh in dev/hither, from hither-sync.manifest.json. " + $note)
       }')
